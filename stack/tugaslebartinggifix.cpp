...
int main() {
    ...
    for (int i = 0; i < jumlahTumpukan; i++) {
        ...
        cout << "|" <<namaTumpukan[i] <<" "<<setfill(' ')<<setw((lebarTumpukan[i]-namaTumpukan[i].length())+1) <<"|\n";
        cout << "|L: " << left << setw(lebarTumpukan[i]-4) << lebarTumpukan[i] << setfill(' ') << setw(lebarTumpukan[i]) << " |" << endl;
